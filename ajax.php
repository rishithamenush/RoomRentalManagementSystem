...
function request_booking() {
    ...
    $overlap_stmt->bind_param("issss", $listing_id, $end_date ?: $start_date, $start_date, $start_date, $end_date ?: $start_date);
    $overlap_stmt->execute();
    $overlap_result = $overlap_stmt->get_result();
    $overlap_count = $overlap_result->fetch_assoc()['count'];
    
    if ($overlap_count > 0) {
        return ['status' => 'error', 'message' => 'This property is not available for the selected dates'];
    }
    ...
    if ($booking_stmt->execute()) {
        ...
        return ['status' => 'error', 'message' => 'Failed to create booking request'];
    }
}
...
function send_message() {
    ...
    $insert_stmt->bind_param("iis", $thread_id, $sender_id, $message_body);
    
    if ($insert_stmt->execute()) {
        // Log audit
        $audit_sql = "INSERT INTO audit_logs (actor_user_id, action, entity, entity_id, meta) VALUES (?, 'send_message', 'message_thread', ?, ?)";
        $audit_stmt = $conn->prepare($audit_sql);
        $meta = json_encode(['thread_id' => $thread_id, 'message_length' => strlen($message_body)]);
        $audit_stmt->bind_param("iis", $sender_id, $thread_id, $meta);
        $audit_stmt->execute();
